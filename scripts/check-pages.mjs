...
try {
  ...
  await page.getByRole('button', { name: 'XLSX＋原始数据', exact: true }).click()
  const download = await pending
  expect(await download.failure()).toBeNull()
  const zip = await JSZip.loadAsync(await readFile(await download.path()))
  expect(zip.file('raw/RAW-Plate-1.xlsx')).not.toBeNull()
  expect(zip.file('qPCR 发布验收-analysis.xlsx')).not.toBeNull()
  const raw = await JSZip.loadAsync(await zip.file('raw/RAW-Plate-1.xlsx').async('uint8array'))
