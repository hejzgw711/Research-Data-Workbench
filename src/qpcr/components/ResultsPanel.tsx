...
function niceAxis(maximum: number) {
  ...
  return { max: count * step, ticks: Array.from({ length: count + 1 }, (_, index) => Number((index * step).toPrecision(12))) }
}
...
export function ResultsPanel({ project, result, geneId, onGeneChange, notify }: {
  ...
  const width = Math.max(550, groups.length * 78)
  const height = 580 + Math.max(0, visibleComparisons.length - 3) * 28
  const left = 68; const right = width - 24; const bottom = height - 86; const top = 96
  const dataMax = Math.max(0.1, ...groups.flatMap(g => g.result ? [g.result.mean + (Number.isFinite(g.result.sd) ? g.result.sd : 0), ...g.result.folds].filter(Number.isFinite) : []))
  const bracketSpace = visibleComparisons.length * 28 + 22
  const axis = niceAxis(dataMax * (bottom - top) / (bottom - top - bracketSpace))
  const spacing = (right - left) / Math.max(groups.length, 1)
  const x = (index: number) => left + spacing * (index + 0.5)
  const y = (value: number) => bottom - (value / axis.max) * (bottom - top)
  ...
  const exportChart = async (format: 'png' | 'svg') => {
    try { if (svgRef.current) await exportSvgElement(svgRef.current, `${project.name}-${target?.name}-模拟表达量`, format); notify(`${format.toUpperCase()} 图片已导出`) } catch (error) { notify(String(error)) }
  }
  const copy = async () => {
    ...
    const text = tableView === 'fold' ? summaryTSV(project, result, geneId) : [
      ['SIMULATED DATA / 模拟数据', target?.name ?? geneId],
  ...
  return <aside className="results-column results-preview-column">
    <section className="card result-card">
      ...
      <div className="expression-chart">
        <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" role="img" aria-label={`${target?.name} 相对表达量柱状图`} viewBox={`0 0 ${width} ${height}`} width={width} height={height} style={{ width: '100%', height: 'auto', minWidth: groups.length > 8 ? width : undefined, display: 'block' }} fontFamily="Arial, Microsoft YaHei, sans-serif" fontSize="12" fill="#181818">
          <rect width={width} height={height} fill="white"/>
          {axis.ticks.map(tick => <g key={tick} data-testid="expression-y-tick"><line x1={left - 5} y1={y(tick)} x2={left} y2={y(tick)} stroke="#171717" strokeWidth="1"/><text x={left - 10} y={y(tick) + 4} textAnchor="end">{formatNumber(tick, 8)}</text></g>)}
          <path d={`M${left} ${top} V${bottom} H${right}`} stroke="#171717" strokeWidth="1.5" fill="none"/>
          ...
          {groups.map((g, i) => {
            ...
            return <g key={g.id}>
              {value && Number.isFinite(value.mean) ? <>
                <rect x={x(i) - barW / 2} y={y(value.mean)} width={barW} height={Math.max(0, bottom - y(value.mean))} fill={palette[i % palette.length]} stroke="#1b1b1b" strokeWidth="1.6"/>
                <line x1={x(i)} y1={y(Math.max(0, value.mean - errorSD))} x2={x(i)} y2={y(value.mean + errorSD)} stroke="#171717" strokeWidth="1.3"/>
                {[Math.max(0, value.mean - errorSD), value.mean + errorSD].map((v, j) => <line key={j} x1={x(i) - 7} x2={x(i) + 7} y1={y(v)} y2={y(v)} stroke="#171717" strokeWidth="1.3"/>)}
              ...
              <text transform={`translate(${x(i) + 4} ${bottom + 15}) rotate(-42)`} textAnchor="end" fontSize="12"><title>{g.name}</title>{g.name.length > 18 ? `${g.name.slice(0, 17)}…` : g.name}</text>
            </g>
          })}
          {visibleComparisons.map((comparison, i) => {
            const a = x(groups.findIndex(g => g.id === comparison.leftGroupId)); const b = x(groups.findIndex(g => g.id === comparison.rightGroupId)); const cy = y(dataMax) - 22 - i * 28
          ...
          <text x={right} y={height - 9} textAnchor="end" fontSize="9" fill="#89928f">SIMULATED · {project.replicateMode === 'biological' ? 'biological samples' : 'technical preview'}</text>
        </svg>
      </div>
